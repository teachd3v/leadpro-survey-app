...
export default async function DashboardPage({ params }) {
  ...
  respondents.forEach(r => {
    ...
    const pesanTerlibat = r[29];
    const kritikPelaksanaan = r[30];
    const saranKeberlanjutan = r[31];
    
    if ((pesanTerlibat && pesanTerlibat.trim() !== '') || 
        (kritikPelaksanaan && kritikPelaksanaan.trim() !== '') || 
        (saranKeberlanjutan && saranKeberlanjutan.trim() !== '')) {
      saranList.push({
        pesanTerlibat,
        kritikPelaksanaan,
        saranKeberlanjutan
